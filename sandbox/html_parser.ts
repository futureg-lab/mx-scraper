...
console.log(
  parser.select("span").eval("attr.class = @reg /primary/").map((_) =>
    _.asText()
  ),
);
console.log(
  parser.select("input").eval("attr.type = text").all().map((_) => _.asValue()),
);
console.log(
  parser.select("input").eval('value = "%John%"').all().map((_) => _.asValue()),
);
console.log(
  parser.select("span").eval("text = @reg /One|Two/i").all().map((_) =>
    _.asText()
  ),
);
