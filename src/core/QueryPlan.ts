    ...
    const filterProcessor = (filter: any, path: string[] = ["filter"]) => {
      ...
      if (filter.linkModifier) {
        ...
        if (Array.isArray(filter.linkModifier)) {
          throw Error(
            `linkModifier should ba an object at ${curr_path}, got array`,
          );
        }
      }
    ...
    const iterateProcessor = (iterate: any, path: string[] = ["iterate"]) => {
      ...
      if (counter.onError) {
        ...
        if (!valid.includes(counter.onError)) {
          ...
          throw Error(
            `onError has invalid value "${counter.onError}" at ${curr_path}, ${expected} expected`,
          );
        }
      }
      if (counter.range) {
        if (Array.isArray(counter.range)) {
          ...
          if (isNaN(start)) {
            throw Error(`${start} is not a number at ${curr_path}.range[0]`);
          }
          if (isNaN(end)) {
            throw Error(`${end} is not a number at ${curr_path}.range[1]`);
          }
          if (start > end) {
            throw Error(`${start} > ${end} at ${curr_path}.range`);
          }
        } else {
